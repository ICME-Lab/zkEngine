...
impl TryFrom<&RVTraceRow> for RV32I {
    ...
    fn try_from(row: &RVTraceRow) -> Result<Self, Self::Error> {
        match row.instruction.opcode {
            ...
            // lhs immediate
            WASM::I32SUBILHS => Ok(SUBInstruction::<WORD_SIZE>(row.imm_u32() as u64, row.register_state.rs1_val.unwrap()).into()),
            // i64
            ...
            // lhs immediate
            WASM::I64SUBILHS => Ok(SUBInstruction::<WORD_SIZE_1>(row.imm_u64(), row.register_state.rs1_val.unwrap()).into()),
            ...
            WASM::I32GEU => Ok(BGEUInstruction::<WORD_SIZE>(row.register_state.rs1_val.unwrap(), row.register_state.rs2_val.unwrap()).into()),
            WASM::I32GES  => Ok(BGEInstruction::<WORD_SIZE>(row.register_state.rs1_val.unwrap(), row.register_state.rs2_val.unwrap()).into()),
            // i64
            ...
            WASM::I64GEU => Ok(BGEUInstruction::<WORD_SIZE_1>(row.register_state.rs1_val.unwrap(), row.register_state.rs2_val.unwrap()).into()),
            WASM::I64GES  => Ok(BGEInstruction::<WORD_SIZE_1>(row.register_state.rs1_val.unwrap(), row.register_state.rs2_val.unwrap()).into()),
            ...
            WASM::I64VIRTUAL_ASSERT_LTE => Ok(ASSERTLTEInstruction::<WORD_SIZE_1>(row.register_state.rs1_val.unwrap(), row.register_state.rs2_val.unwrap()).into()),
            WASM::I64VIRTUAL_ASSERT_VALID_UNSIGNED_REMAINDER => Ok(AssertValidUnsignedRemainderInstruction::<WORD_SIZE_1>(row.register_state.rs1_val.unwrap(), row.register_state.rs2_val.unwrap()).into()),
