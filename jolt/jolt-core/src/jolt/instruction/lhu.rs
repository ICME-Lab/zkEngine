...
use tracer::{ELFInstruction, MemoryState, RVTraceRow, RegisterState, WASM};
...
impl<const WORD_SIZE: usize> VirtualInstructionSequence for LHUInstruction<WORD_SIZE> {
    ...
    fn virtual_trace(trace_row: RVTraceRow) -> Vec<RVTraceRow> {
        assert_eq!(trace_row.instruction.opcode, WASM::LHU);
        ...
        virtual_trace.push(RVTraceRow {
            instruction: ELFInstruction {
                ...
                opcode: WASM::VIRTUAL_ASSERT_HALFWORD_ALIGNMENT,
                rs1,
                rs2: None,
                rd: None,
                imm: Some(offset),
